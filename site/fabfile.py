...
env.uwsgi_app_path = os.path.dirname(os.path.realpath(__file__)) + '/../config/uwsgi-app/files'
env.environment_settings_path = os.path.dirname(os.path.realpath(__file__)) + '/../config'
...
def production():
    ...
    env.remote_project_path = '/home/django/djangoberry/'
    env.deploy_archive_path = '/home/'
    env.virtualenv_path = '/home/django/.virtualenvs/djangoberry-live-venv/'
    ...
    env.start_service = 'service nginx start'
    env.stop_service = 'service nginx stop'
    env.light_restart = "service nginx reload"
...
def relink():
    ...
    if not env.is_predeploy:
        if files.exists('%s/%s' % (version_path, env.SHA1_FILENAME)): # check the sha1 dir exists
            ...
            virtualenv('ln -s %s/%s/dist %s' % (version_path, env.SHA1_FILENAME, project_path,)) # relink
...
def do_deploy():
    ...
    if not files.exists('%s/index.html' % full_version_path):
        unzip_archive()
...
def deploy_settings():
    ...
    put(local_path='%s/.production-env' % (env.environment_settings_path,), remote_path='%s/.env-%s-production' % (env.deploy_archive_path, env.project,))
    sudo('cp %s/.env-%s-production %s/.env' % (env.deploy_archive_path, env.project, full_version_path))
    run('chown -R %s:%s %s/.env' % (env.application_user, env.application_user, full_version_path) )
